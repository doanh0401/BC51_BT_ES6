...
domId("btnThemSP").onclick = () => {
  ...
  document.querySelector(
    ".modal-footer"
  ).innerHTML = `<button onclick="AddPerson()" class="btn btn-success">Add</button>`;
  domId("myForm").reset();
  clearError(
    "errorName",
    "errorAddress",
...
window.HiddenInput = () => {
  ...
  domId("diemLy").value = "";
  domId("diemHoa").value = "";
  domId("ngaylamviec").value = "";
  domId("luongngay").value = "";
...
window.AddPerson = () => {
  ...
  if (checkValidation(true)) {
    const isValue = validation.checkId(
      person.id,
      "errorId",
...
function createEntity(person) {
  if (person.typeJob === "Student") {
    ...
    const { name, address, id, email, typeJob, ngaylamviec, luongngay } =
      person;
...
window.OpenEdit = (id) => {
  ...
  document.querySelector(
    ".modal-footer"
  ).innerHTML = `<button onclick="EditPerson()" class="btn btn-success">Edit</button>`;
